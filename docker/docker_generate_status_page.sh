...
#
#   DOCKER_USER=buildscale ./github_generate_status_docker.sh
#
...
#
#   DOCKER_USER=buildscale ./github_generate_status_page.sh  buildscale/hbase buildscale/zookeeper buildscale/nagios-plugins ...
#
#   DOCKER_USER=buildscale ./github_generate_status_page.sh  hbase zookeeper nagios-plugins ...
...
if [ -z "$repolist" ]; then
    repolist="$(dockerhub_search.py -n 100 buildscale | awk '/^buildscale\/.*[[:space:]]\[OK]/{print $1}' | sort)"
fi
...
generated by \`${0##*/}\` in [BuildScale/DevOps-Scripts](https://github.com/BuildScale/DevOps-Scripts)
