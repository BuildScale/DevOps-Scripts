...
#  args: buildscale/devops-scripts testenv haritest=stuff haritest2=stuff2
#
...
Examples:
    ...
    ${0##*/} BuildScale/DevOps-Scripts testenv AWS_ACCESS_KEY_ID=AKIA...

    echo AWS_ACCESS_KEY_ID=AKIA... | ${0##*/} BuildScale/DevOps-Scripts testenv

    echo AWS_ACCESS_KEY_ID=AKIA... | gh secret set -R BuildScale/DevOps-Scripts --env testenv -f -


    Loads both AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY via stdin:

        aws_csv_creds.sh credentials_exported.csv | ${0##*/} BuildScale/DevOps-Scripts testenv

        aws_csv_creds.sh credentials_exported.csv | gh secret set -R BuildScale/DevOps-Scripts --env testenv -f -
