...
{owner} - repo owner eg. BuildScale
{repo}  - repo name without the user/org prefix (eg. DevOps-Scripts)
...
if [ -n "$repolist" ]; then
    ...
    repolist="$(curl -sSL https://raw.githubusercontent.com/BuildScale/bash-tools/master/setup/repos.txt | sed 's/#.*//')"
fi
